    ...
    <div class="login-form-side">
      <div class="login-form-wrap">
        ...
        <div class="form-header">
          ...
          <h1>Inicia sesión</h1>
          <p>¿No tienes cuenta? <router-link to="/signup" class="link-pink">Regístrate aquí</router-link></p>
        </div>
        ...
        <form class="form" @submit.prevent="handleLogin" novalidate>
          ...
          <div class="form__forgot">
            <a href="#" class="link-pink" @click.prevent="forgotPassword">¿Olvidaste tu contraseña?</a>
          </div>
        ...
        <p class="form-footer">
          ...
          <router-link to="/legal" class="link-pink">Términos y condiciones</router-link>
        </p>
...
.deco-blob--2 { width: 300px; height: 300px; background: rgba(255,255,255,.4); bottom: -40px; left: -40px; animation-delay: 3s; }
.deco-blob--3 { width: 200px; height: 200px; background: rgba(194,24,91,.25);  top: 50%;    left: 20%;   animation-delay: 6s; }
...
.deco__content {
  ...
  gap: 28px; text-align: center; padding: 40px 32px;
}
...
.deco__logo {
  ...
  text-shadow: 0 4px 20px rgba(194,24,91,.3);
}
.deco__logo-icon {
...
.deco__tagline {
  ...
  font-size: 1.3rem; color: rgba(255,255,255,.9); line-height: 1.5;
}

.deco__cards { display: flex; gap: 14px; }
.deco__card {
...
.deco__card:hover { transform: translateY(-8px) scale(1.05); }
.deco__card span  { font-size: 2rem; }
.deco__card small { font-size: 0.72rem; color: rgba(255,255,255,.9); font-weight: 600; letter-spacing: 0.5px; }
.deco__card--1 { animation: floatCard 4s ease-in-out infinite; }
.deco__card--2 { animation: floatCard 4s ease-in-out 0.8s infinite; }
.deco__card--3 { animation: floatCard 4s ease-in-out 1.6s infinite; }
...
.deco__stat {
  ...
  font-size: 0.85rem; color: rgba(255,255,255,.95); font-weight: 500;
}
.deco__stat strong { font-size: 1.1rem; color: #fff; }
...
.login-form-wrap {
  ...
  animation: formIn 0.6s cubic-bezier(.34,1.56,.64,1) both;
}
@keyframes formIn {
...
.form-header__tag {
  display: inline-block;
  background: rgba(233,30,140,.1); color: var(--pink-deep);
...
.form-header h1 {
  font-family: 'Playfair Display', serif;
  font-size: 2.2rem; color: var(--text); line-height: 1.1;
}
...
.field__input-wrap {
  ...
  background: #fff; border: 1.5px solid rgba(233,30,140,.2);
...
.field__check { color: #22c55e; font-size: 0.9rem; font-weight: 700; flex-shrink: 0; }
.field__eye { background: none; border: none; cursor: pointer; font-size: 0.9rem; padding: 0 2px; flex-shrink: 0; }
.field__error { font-size: 0.78rem; color: #ef4444; padding-left: 4px; }

/* ── Olvidé contraseña ─────────────────────────────────────── */
.form__forgot { text-align: right; margin-top: -6px; }
.form__forgot a { font-size: 0.82rem; }

/* ── Error global ──────────────────────────────────────────── */
.global-error {
  background: rgba(239,68,68,.08); border: 1px solid rgba(239,68,68,.2);
  color: #b91c1c; border-radius: 12px; padding: 12px 16px;
...
.btn-submit {
  ...
  box-shadow: 0 8px 28px rgba(233,30,140,.3);
...
.divider::before, .divider::after {
  content: ''; flex: 1; height: 1px; background: rgba(233,30,140,.15);
}
...
.btn-google {
  ...
  cursor: not-allowed; width: 100%; transition: all 0.2s;
}
.btn-google small { font-size: 0.75rem; }

/* ── Footer ────────────────────────────────────────────────── */
.form-footer {
  text-align: center; font-size: 0.78rem; color: var(--text-light); line-height: 1.6;
}
